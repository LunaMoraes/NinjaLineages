...
function Authority.execute(player, requestId, actionId, args)
    ...
    if result.ok and actionId ~= "nature_chakra_manipulation" and actionId ~= "sage_mode" and NinjaLineages.SageMode and NinjaLineages.SageMode.onCastJutsu then
...
function Authority.handleResult(result)
    ...
    if result.ok then
        if result.state and result.state.eyePowerActive ~= nil then
            ...
            if (isDebugEnabled and isDebugEnabled())
                    or (SandboxVars and SandboxVars.NinjaLineages and SandboxVars.NinjaLineages.DebugMode == true) then
                print(string.format(
